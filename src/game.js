...
let w = window.innerWidth - 2, h = window.innerHeight - 2
...
let gameStart = false
let main_thread_id
let level_symbol = [], level = 1, count_sy
let swapn_list = []
let score = 0
let create_sy_id
let symbol_data = [
    { name: 'xuống', speed: 8, hp: 1, score: 100, img: '/res/symbol_01.png', w: 30, h: 30 },
    { name: 'lên', speed: 12, hp: 1, score: 200, img: '/res/symbol_02.png', w: 30, h: 30 },
    { name: 'trái', speed: 10, hp: 1, score: 150, img: '/res/symbol_03.png', w: 30, h: 30 },
    { name: 'phải', speed: 8, hp: 1, score: 100, img: '/res/symbol_04.png', w: 30, h: 30 }]
...
function createLevel(level, maxQt) {
    let list = []
    ...
    for (var i = 0; i < maxQt; i++) {
        ...
        if (percent_hp < percent_hp_hight)
            data.hp = MathRandom(1, level)
...
function spawnSymbol(spawn_time) {
    let create_sy_id;
    ...
    create_sy_id = setInterval(() => {
        ...
        let sy = new GameSymbol(x.name, x.speed,
            x.position, x.hp,
            x.score, x.img, x.w, x.h)
        sy.onUpdate = () => {
            if (sy.position.y > h) {
                sy.destroy()
...
function recogintionToObject(array) {
    let list_match = [];
